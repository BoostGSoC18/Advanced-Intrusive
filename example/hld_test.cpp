...
void check(int cur,Graph &graph,bool visited[],int subtree_size[],int heavy_child[],bool result[])
{
    ...
    cout << maxi << " "  << subtree_size[heavy_child[cur]] << "\n";
...
int main()
{
    ...
    Graph_list copy_graph;
    copy_graph=graph_list;
    graph_traits<Graph_list>::edge_iterator st,en;
    std::pair<graph_traits<Graph_list>::edge_iterator,graph_traits<Graph_list>::edge_iterator> es = boost::edges(copy_graph);
    ...
    // int node=first.get_chain_head(2);
    // cout << node << "\n";
